...
if __name__ == "__main__":
    ...
    def init_torch_linear(A_shape, B_shape, weight):
        A = torch.randn(A_shape[0], A_shape[1], dtype=torch.float16, device="cuda")
        torch_linear = torch.nn.Linear(B_shape[1], B_shape[0], bias=False).cuda()
        torch_linear.weight.data = weight
        return {"A": A, "torch_linear": torch_linear}

    def run_torch_linear(A, torch_linear):
        torch_linear(A)
    ...
    A_shape = (8192, 4096)
    B_shape = (4096, 4096)
    weight = torch.randn(B_shape[1], B_shape[0], dtype=torch.float16, device="cuda")
    init_params = {
        "default": {"A_shape": A_shape, "B_shape": B_shape, "weight": weight},
        "torch_linear": {},
    }
